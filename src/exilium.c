...
/* Hay que copiar los archivos de allegro/setup LINUX en setup, y compilar   */
/* Copiar el allegro-config compilado de /allegro en /usr/bin                */
/*                                                                           */
/* Nota: El jgmod va despues de las allegro, ya que sino no suena sonido     */
/* Autor: Jaime Jose Gavin Sierra.  Alias: J.J.                              */
/*                                                                           */
/* En BeOS                                                                   */
/* Compila: gcc -s -O6 exilium.c -obeosexilium -lalleg-4.1.4                 */
/* Copiar los archivos allegro/setup BeOS en setup                           */
/*                                                                           */
/* En QNX                                                                    */
/* Compila: gcc -s -O6 exilium.c -oqnxexilium `allegro-config --libs`        */
/* Copiar los archivos allegro/setup QNX en setup                            */
/* Advertencia: En QNX no se ha puesto el SETUP, por dar error compilar      */
/*                                                                           */
/* En DOS:                                                                   */
/* Compila: gcc -s -O6 exilium.c -oexilium.exe -lalleg                       */
/* Copiar los archivos allegro/setup DOS en setup                            */
/*                                                                           */
/* En WIN32:                                                                 */
/* Compila: gcc -s -O6 exilium.c -owinexilium.exe -lalleg                    */
/* Copiar los archivos allegro/setup WIN32 o DOS en setup                    */
/*****************************************************************************/
...
void MostrarErrorMemoria(void);
void CapturarPantalla(void);
...
int main(int argc,char *argv[]){
 ...
 valorRaton = install_mouse();
 if ((valorRaton==0)||(valorRaton==-1)) raton=FALSE; //Instalo el raton
 else raton=TRUE;


 install_timer();         //Para poder mostrar raton activo el timer

 set_window_title("Diskmag Exilium");

 if (install_sound (DIGI_AUTODETECT, MIDI_AUTODETECT, NULL) != 0){
   //allegro_message ("Error initializing sound card");
   sonido = driverSonido = FALSE;
 }
 else
...
// driverSonido =FALSE;
 ...
 packfile_password(cadenaClave);
  ...
  LeerArchivoDatos("sliders2.dat",&revista);
  LeerArchivoDatos("config.dat",&configuracion);
 ...
 letras = create_bitmap(anchoX,altoY);
 if (letras==NULL) MostrarErrorMemoria();
 auxBitmap = create_bitmap(anchoX,altoY);
 if (auxBitmap==NULL) MostrarErrorMemoria();
 videoTemporal = create_bitmap(anchoX,altoY);
 if (videoTemporal==NULL) MostrarErrorMemoria();
 clear_to_color(letras,bitmap_mask_color(letras));
 if (set_gfx_mode(GFX_AUTODETECT,anchoX,altoY,0,0)!=0){
  allegro_message("Error en modo de video\nCambie driver grafico en CFG\nO pruebe en escala de grises\n");
  allegro_exit();
  //printf("Error en modo de video\nCambie driver grafico en CFG\nO pruebe en escala de grises");
  exit(-1);
 ...
 if (raton!=FALSE){
  show_mouse(NULL);
  set_mouse_sprite(configuracion[FOTO_RATON].dat);
  set_mouse_sprite_focus(15,0);
  position_mouse(anchoX-(anchoX/4),altoY-(altoY/7));
  show_mouse(screen);                       //Muestro raton
 }
...
// hline(fondo,0,400,639,makecol(255,255,255));
 ...
 set_gfx_mode(GFX_TEXT,80,25,0,0);//Restablezco el modo de video
...
/* set_gfx_mode(GFX_TEXT,80,25,0,0);
 ...
 if (raton!=FALSE) show_mouse(NULL);
 remove_timer;
 ...
 *datos = load_datafile(nombre);
 if (*datos==NULL) {
   strcpy(cadena,"Error leyendo el archivo de datos: ");
...
void MostrarArticulo(DATAFILE * indice){
 ...
 if (indice!=NULL){
  ...
  for (i=1;i<=numFotos;i++){//Pongo todas las fotos
   ...
   if (indiceFoto==NULL){
    allegro_message("Error leyendo indice de foto\n");
    allegro_exit();
    exit(-1);
   }
 ...
 // elArticulo.numFotos=(unsigned char)indiceFoto;
   ...
   if (indicePaleta==NULL){
    allegro_message("Error leyendo indice de la paleta de la foto\n");
    allegro_exit();
    exit(-1);
   }
...
void LeerArticulos(TArticulos *articulos, DATAFILE *revista){
 ...
 for (i=1;i<=numArticulos;i++){//Pongo todos los articulos
  ...
  if (find_datafile_object(revista,cadAux)==NULL){
   allegro_message("Error leyendo los indices de articulos\n");
   allegro_exit();
   exit(-1);
  }
  else
...
void MostrarBotones(){
 ...
 if ((sonido==FALSE)||(driverSonido==FALSE)){
  for (i=0;i<3;i++){
...
// rectfill(screen,0,0,319,199,255);

 crono = 0;
 LOCK_VARIABLE(crono);
 LOCK_FUNCTION(inc_crono);
 install_int(inc_crono,1000); //Arranco el temporizador de segundos
 ...
 auxiliar1 = create_bitmap (300,55);
 auxiliar2 = create_bitmap (300,55);
 blit (screen,auxiliar1,0,20,0,0,300,55);
 blit (auxiliar1,auxiliar2,0,0,0,0,300,55);

 for (i=-100;i<30;i+=2){
  masked_blit (diskmag,auxiliar2,0,0,i,0,diskmag->w,diskmag->h);
  vsync();
  blit (auxiliar2,screen,0,0,0,20,auxiliar2->w,auxiliar2->h);
  blit (auxiliar1,auxiliar2,0,0,0,0,auxiliar2->w,auxiliar2->h);
 }
 while ((crono<7)&&!keypressed()); //7 segundos


 blit (screen,auxiliar1,0,75,0,0,300,55);
 blit (auxiliar1,auxiliar2,0,0,0,0,300,55);
 
 for (i=-100;i<80;i+=2){ 
  masked_blit(exilium,auxiliar2,0,0,i,0,exilium->w,exilium->h);
  vsync();
  blit (auxiliar2,screen,0,0,0,75,auxiliar2->w,auxiliar2->h);
  blit (auxiliar1,auxiliar2,0,0,0,0,auxiliar2->w,auxiliar2->h);
 }
 while ((crono<10)&&!keypressed()); //10 segundos


 blit (screen,auxiliar1,0,170,0,0,300,55);
 blit (auxiliar1,auxiliar2,0,0,0,0,300,55);

 for (i=-50;i<190;i+=2){
  masked_blit (issue,auxiliar2,0,0,i,0,issue->w,issue->h);
  vsync();
  blit (auxiliar2,screen,0,0,0,170,auxiliar2->w,auxiliar2->h);
  blit (auxiliar1,auxiliar2,0,0,0,0,auxiliar2->w,auxiliar2->h);  
 }

 destroy_bitmap(auxiliar1);
 destroy_bitmap(auxiliar2);

 while ((crono<13)&&!keypressed()); //Espera a fundir a los 13 segundos
 //fade_out(1);
...
void Despedida(DATAFILE *datos){
 ...
 blit(foto,screen,0,0,0,0,foto->w,foto->h);
 masked_blit(exilium,screen,0,0,80,40,exilium->w,exilium->h);
 masked_blit(issue,screen,0,0,190,170,issue->w,issue->h);
 crono=0;
 LOCK_VARIABLE(crono);
 LOCK_FUNCTION(inc_crono);
 install_int(inc_crono,1000); //Arranco el temporizador de segundos
 ...
 while ((crono<7)&&(!keypressed())); //8 segundos
...
/************************************************/
void MostrarLineaComandos(void){
 printf("\n Diskmag Exilium issue #2   Grupo: SLIDERS   Fecha: 9-1-2003\n");
 printf("==============================================================\n\n");
...
void ControlaEventoTeclado(boolean * salir){
 ...
 if (keypressed()){
  ...
  switch(scancode){
   ...
   case KEY_F12: CapturarPantalla(); break;
   case KEY_F10: CapturarPantalla(); break;
...
/*****************************************/
void MostrarErrorMemoria(void){
 allegro_message("No hay memoria RAM suficiente\n");
 allegro_exit();
 //printf("No hay memoria RAM suficiente");
 exit(-1);
}
...
void CapturarPantalla(){
 BITMAP *pantalla;
 pantalla = create_bitmap(anchoX,altoY);
 if (pantalla!=NULL){
  if (raton) show_mouse(NULL);
  blit (screen,pantalla,0,0,0,0,anchoX,altoY);
  if (grises)
   save_pcx("screen.pcx",pantalla,paletaGris);
  else
   save_pcx("screen.pcx",pantalla,NULL);
  if (raton) show_mouse(screen);
